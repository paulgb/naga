...
fn main([[builtin(global_invocation_id)]] global_invocation_id: vec3<u32>) {
    ...
    loop {
        ...
        let _e42: u32 = i;
        pos1 = particlesSrc.particles[_e42].pos;
        let _e47: u32 = i;
        vel1 = particlesSrc.particles[_e47].vel;
        let _e51: vec2<f32> = pos1;
        let _e52: vec2<f32> = vPos;
        if ((distance(_e51, _e52) < params.rule1Distance)) {
        ...
        if ((distance(_e63, _e64) < params.rule2Distance)) {
        ...
        if ((distance(_e74, _e75) < params.rule3Distance)) {
    ...
    vVel = (((_e107 + (_e108 * params.rule1Scale)) + (_e113 * params.rule2Scale)) + (_e118 * params.rule3Scale));
    ...
    vPos = (_e131 + (_e132 * params.deltaT));
